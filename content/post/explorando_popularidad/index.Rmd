...
Nótese que usamos algunas de las funciones que intridujimos en un [post anterior](../preparacion_funciones), que desde ya hacen uso y abuso de la gramática de `ggplot`
...
gobernadores_rtspromedio_porcentaje <- plotPoint(gobernadores_popu_ranking, 
...
presid_rtspromedio_porcentaje <- plotPoint(presid_popu_ranking, 
       aes(rts_obtenidos_promedio, 
           Porcentaje, 
           colour = screen_name)) + 
    labs(title = "a Presidente", 
         x = "Rts obtenidos promedio", 
         y = "% de votos obtenidos")
...
```{ rg2, warning= FALSE, message= FALSE}
...
Querés explorar un poco más en las palabras y temas abordados por nuestros emisores? :fast_forward: seguime en el [siguiente post](../exploracion_palabras/).
